...
if __name__ == '__main__':
    ...
    if not os.path.isfile(opt.image_path):            #目录
        ...
        for img_path in file_list:             #遍历图片文件
            ...
            print(f"\nProcessing image [{count}]: {img_path}") # 修改了打印格式
            ...
            if not dict_list:
                ...
                for i, result in enumerate(dict_list):
                    plate_type_str = "双层" if result['plate_type'] == 1 else "单层"
                    print(f"    车牌 {i+1}: {result['plate_no']}")
                    if opt.is_color:
                        color_conf_str = f"{result.get('color_conf', 0.0):.2f}"
                        print(f"      颜色: {result['plate_color']} (置信度: {color_conf_str})")
                    print(f"      类型: {plate_type_str}")
                    print(f"      检测置信度: {result['detect_conf']:.2f}")

            time_e=time.time()
            time_gap = time_e-time_b                         #计算单个图片识别耗时
            print(f"... 耗时: {time_gap:.4f}s")
            if count:
                time_all+=time_gap 
            count+=1
        print(f"sumTime time is {time.time()-time_begin} s, average pic time is {time_all/(len(file_list)-1)}")
    else:                                          #单个图片
            print(f"Processing single image: {opt.image_path}")
            time_b = time.time()
