...
def intersect(box_a, box_b):
    ...
    Args:
        box_a: [batch, num_boxes, 4] 格式为 [x1, y1, x2, y2]
        box_b: [batch, num_boxes, 4] 格式为 [x1, y1, x2, y2]
    
    Returns:
        inter: [batch, A, B] 交集面积张量
    """
    # 扩展维度以便广播计算
    # box_a: [batch, A, 4] -> [batch, A, 1, 4]
    # box_b: [batch, B, 4] -> [batch, 1, B, 4]
    A = box_a.size(1)
    B = box_b.size(1)
    
    # 获取交集的坐标
    max_xy = torch.min(box_a[:, :, 2:].unsqueeze(2).expand(-1, A, B, 2),
...
def jaccard_diou(box_a, box_b, iscrowd:bool=False):
    """
    计算 Jaccard-DIoU (Distance Intersection over Union)
    
    Args:
        box_a: 边界框A，格式为 [x1, y1, x2, y2]
        box_b: 边界框B，格式为 [x1, y1, x2, y2]
        iscrowd: 是否处理crowd场景
    
    Returns:
        jaccard_diou: Jaccard-IoU 减去距离惩罚项
    """
    use_batch = True
    if box_a.dim() == 2:
        use_batch = False
